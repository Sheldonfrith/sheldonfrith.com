...
function assertIsAnimal(a: any): asserts a is Animal{
    if (!(objectKeys(a).includes('numberOfFeet')) || !(objectKeys(a).includes("sound"))){
//     throw new Error("assertIsString failed, is not a string: "+s);
    }
}

function funcThatCanTakeAnythingButCanOnlyDoSomethingIfItsAnAnimal<T>(arg: CheckTypeIsAnimal<T>){
